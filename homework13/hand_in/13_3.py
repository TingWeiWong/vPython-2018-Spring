...
scene=display(title='dipole',height=500,width=500,range=3.5,auto_scale=False,background=(0.5,0.5,0.4))
...
for i in range(12):
    for j in range(6):
        ball.append(sphere(pos=vector(0.1*cos(60*j)*cos(30*i),L+0.1*cos(60*j)*sin(30*i),0.1*sin(60*j)),radius=0.005,color=(1,1,1),make_trail=true,v=vector(0,0,0),a=vector(0,0,0),q=ball_q,f=vector(0,0,0)))
for i in range(12):
    for j in range(6):
        ball.append(sphere(pos=vector(0.1*cos(60*j)*cos(30*i),-L+0.1*cos(60*j)*sin(30*i),0.1*sin(60*j)),radius=0.005,color=(1,1,1),make_trail=true,v=vector(0,0,0),a=vector(0,0,0),q=ball_q,f=vector(0,0,0)))
...
while true:
    ...
    for items in ball:
        ...
        if abs(items.pos-vector(-L,0,0))<=size or abs(items.pos-vector(L,0,0))<=size:
