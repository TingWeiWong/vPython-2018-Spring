from visual import *
k=8.99*10**9
L,size=1.0,0.1 #pole lenghth charge size 
polecharge,poleposition,polecolor=[1E-5,-1E-5],[vector(L,0,0),vector(-L,0,0)],[(1,0,0),(0,0,1)]
ball_m, ball_q = 1E-6, 1E-12 #kg(mass of ball), C(charge of ball)
scene = display(title='dipole', height=500, width=500, range=3.5,background=(0.5,0.3,0.2))
pole_set = [sphere(pos=p, radius=size, color= cc, q = charge) for (charge, p, cc) in zip(polecharge, poleposition, polecolor)] 
ball = []
label(text='Left click to place balls.', pos=(0,0.5,0), opacity=0.2)
def Force_E(r, q):
	r0=r-poleposition[0]
	r1=r-poleposition[1]
	return k*1E-5*q/abs(r0)**2*r0.norm()-k*1E-5*q/abs(r1)**2*r1.norm()
def makeSphere(evt):
	loc = evt.pos
	print ("click at ", loc)
	ball.append(sphere(pos=loc, radius=0.05, color=(1,0,1), make_trail=True, v=vector(0,0,0), a=vector(0,0,0), q=ball_q))
	print Force_E(loc,ball_q)
scene.bind('mousedown', makeSphere, scene) #2
dt = 0.001 
while true:
    rate(3000)
    for items in ball:
        items.a=Force_E(items.pos,ball_q)/ball_m
        items.v+=items.a*dt
        items.pos+=items.v*dt
        if abs(items.pos-vector(-L,0,0))<=0.1*size:
            items.make_trail=false
            items.visible=false