...
inductors=[]
for i in range(NL):
    inductors.append(cylinder(pos=vector(dis*cos(2*pi*i/NL),dis*sin(2*pi*i/NL),0),axis=(1*cos(2*pi*i/NL),1*sin(2*pi*i/NL),0),radius=R))
...
while(NL<=20):
    ...
    while(t<1):
        ...
        MagS.axis = -Mag.axis
    text="V="+str(Vp)+' NL='+str(NL)
    document.write(text)
    document.write('\n')
    NL+=1
    i_.append(gcurve(color=(1-(a/(NL+0.0)),0,(a/(NL+0.0))), gdisplay = scene1))
    for i in range(NL-1):
        inductors[i].pos=vector(dis*cos(2*pi*i/NL),dis*sin(2*pi*i/NL),0)
        inductors[i].axis=(1*cos(2*pi*i/NL),1*sin(2*pi*i/NL),0)
    inductors.append(cylinder(pos=vector(dis*cos(2*pi*(NL-1)/NL),dis*sin(2*pi*(NL-1)/NL),0),axis=(1*cos(2*pi*(NL-1)/NL),1*sin(2*pi*(NL-1)/NL),0),radius=R))
document.close()
